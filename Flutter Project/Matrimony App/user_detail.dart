          ...
          child: Column(
            children: [
                ...
                child: Column(
                  children: [
                    CircleAvatar(
                      ...
                      child: buildLeadingIcon(User.userList[widget.index])
                    ),
                    SizedBox(height: 8),
                    Text("${User.userList[widget.index]["NAME"]}", style: TextStyle(color:Colors.pink[300],fontSize: 18, fontWeight: FontWeight.bold)),
              ...
              buildProfileSection("About", [
                profileRow("Name", "${User.userList[widget.index]["NAME"]}"),
                profileRow("Gender", "${User.userList[widget.index]["GENDER"]}"),
                profileRow("Date of Birth", "${User.userList[widget.index]["DOB"]}"),
              ...
              buildProfileSection("Contact Details", [
                profileRow("Email ID", "${User.userList[widget.index]["EMAIL"]}"),
                profileRow("Phone", "+91 ${User.userList[widget.index]["PHONE"]}"),
              ]),
              ...
              //       print(User.userList[widget.index]);
              //       return AddUser(userDetail: User.userList[widget.index],);
              //     })).then((value) async {
              //       if (value != null) {
              //         await user.updateUser(User.userList[widget.index][MatromonialDatabase.USER_ID],value);
              //         print(User.userList[widget.index][MatromonialDatabase.GENDER]);
              //
              //       }
              //       setState(() {});
              //     });
